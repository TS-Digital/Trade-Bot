/**
 * AUD/USD 4H Swing signal detector (Twelve Data REST API).
...
function checkAUDUSDSwing(candles) {
  if (!isForexOpen()) { console.log('[AUD/USD Swing] Market closed — skipping'); return null; }
  if (!candles || candles.length < LOOKBACK + 10) { console.log('[AUD/USD Swing] Not enough candles'); return null; }
  ...
  console.log(
    `[AUD/USD Swing] Close: ${currentClose.toFixed(4)} | ` +
...
function checkAUDUSDSwingBrewing(candles) {
...
module.exports = { checkAUDUSDSwing, checkAUDUSDSwingBrewing };
