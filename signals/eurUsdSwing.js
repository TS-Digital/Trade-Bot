/**
 * EUR/USD 4H Swing signal detector (Twelve Data REST API).
...
function checkEURUSDSwing(candles) {
  if (!isForexOpen()) { console.log('[EUR/USD Swing] Market closed — skipping'); return null; }
  if (!candles || candles.length < LOOKBACK + 10) { console.log('[EUR/USD Swing] Not enough candles'); return null; }
  ...
  console.log(
    `[EUR/USD Swing] Close: ${currentClose.toFixed(4)} | ` +
...
function checkEURUSDSwingBrewing(candles) {
...
module.exports = { checkEURUSDSwing, checkEURUSDSwingBrewing };
