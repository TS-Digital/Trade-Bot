/**
 * XRP/USD 6H Swing signal detector (Coinbase via CCXT).
 */
...
function checkXRPSwing(candles) {
  if (!candles || candles.length < LOOKBACK + 10) {
    console.log('[XRP/USD Swing] Not enough candles');
    return null;
  }
  ...
  console.log(
    `[XRP/USD Swing] Close: ${currentClose.toFixed(4)} | ` +
  ...
  if (currentClose > resistance && volSpike && rsi !== null && rsi < 70) {
    ...
    return {
      ...
      reason: `Resistance breakout (${resistance.toFixed(4)}) + volume spike`,
    };
  }
  ...
  if (currentClose < support && volSpike && rsi !== null && rsi > 30) {
    ...
    return {
      ...
      reason: `Support breakdown (${support.toFixed(4)}) + volume spike`,
    };
  }
...
function checkXRPSwingBrewing(candles) {
...
module.exports = { checkXRPSwing, checkXRPSwingBrewing };
