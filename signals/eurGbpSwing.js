/**
 * EUR/GBP 4H Swing signal detector (Twelve Data REST API).
...
function checkEURGBPSwing(candles) {
  if (!isForexOpen()) { console.log('[EUR/GBP Swing] Market closed — skipping'); return null; }
  if (!candles || candles.length < LOOKBACK + 10) { console.log('[EUR/GBP Swing] Not enough candles'); return null; }
  ...
  console.log(
    `[EUR/GBP Swing] Close: ${currentClose.toFixed(4)} | ` +
...
function checkEURGBPSwingBrewing(candles) {
...
module.exports = { checkEURGBPSwing, checkEURGBPSwingBrewing };
